...
export function createInputHandler(canvas: HTMLCanvasElement, joystick: JoystickManager): InputHandler {
	const digital = {
		forward: false,
		backward: false,
		left: false,
	...
	joystick.on('end', () => {
		digital.backward = digital.forward = digital.right = digital.left = false;
	});
	joystick.on('dir', (_, data) => {
		switch (data.direction.angle) {
			case 'up':
				digital.forward = true;
				digital.backward = false;
				break;

			case 'down':
				digital.backward = true;
				digital.forward = false;
				break;
